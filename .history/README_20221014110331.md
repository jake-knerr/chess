...
- Order:
  - imports - imports should be alone because they are extraordinary
    theme
    fonts
    variables
    resets
    globals
    utilities - utilities above components because they should never override component rules.components
...
- A component must keep all its rules in a single file. Otherwise create an extended component. order can matter
- inheritance
...
- use rem for fonts (or anything that should scale with fonts)
...
- [Shadow DOM](#shadow-dom)
  - [For the time being, avoid the Shadow DOM.](#for-the-time-being-avoid-the-shadow-dom)
...
Shadow DOM is a great concept, but implementation considerations argue against its use for now.

> Why? Using the shadow DOM requires JavaScript, which is unsuitable for many content-focused projects.

> I am building a SPA, why not use the Shadow DOM? Every element using a shadow DOM requires a style tag in each element or a link to a preloaded stylesheet. Parsing a stylesheet for every component has negative performance implications. A solutions named 'adopted stylesheets' aims to fix this problem, but Safari has not implemented it.
